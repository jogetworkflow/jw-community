...
<#if element.properties.readonly! != 'true'>
    ...
    <div class="grid grid_${element.properties.elementUniqueKey!}" id="${elementParamName!}">
        <table cellspacing="0">
            ...
            <#list headers?keys as header>
                <td><span id="${elementParamName!}_${header}" class="grid-cell">Click to edit</span> <input class="grid-input" type="hidden" value="" /></td>
            </#list>
            </tr>
            <#list rows as row>
                <tr class="grid-row">
                <#list headers?keys as header>
                    <td><span id="${elementParamName!}_${header}" class="grid-cell">${row[header]}</span> <input name="${elementParamName!}_${header}_${row_index}" class="grid-input" type="hidden" value="${row[header]!?html}" /></td>
                </#list>
                </tr>
            </#list>
        </table>
