(function($){
    $.fn.extend({
        passwordField: {
            ...
            togglePasswordVisibility: function (o) {
                const $input = $('#' + o.paramName + '_' + o.elementUniqueKey).closest('.password-input-wrapper').find('input[type="password"], input[type="text"]');
                const $button = $input.siblings('button');
                $button.attr('title', o.messages["form.passwordfield.showPassword"]);

                if ($input.val() !== '') {
                    $button.hide();
                    const detectModify = function() {
                        $button.show();
                        $input.addClass('hasToggle');
                        $input.off('input', detectModify);
                    };
                    $input.on('input', detectModify);
                } else {
                    $input.addClass('hasToggle');
                }
                ...
                $($button).off('click').on('click', function(e) {
                    ...
                    const isPasswordField = $input.attr('type') === 'password';
                    if (isPasswordField) {
                        $input.attr('type', 'text');
                        $button.attr('title', o.messages["form.passwordfield.hidePassword"]);
