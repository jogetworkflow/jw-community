...
<commons:popupHeader />
    ...
    <script type="text/javascript">
        function validateField() {
            ...
            if (!idMatch && oversize) {
                UI.alert('<ul><li><ui:msgEscJS key="console.directory.org.error.label.idInvalid"/></li><li><ui:msgEscJS key="console.directory.org.error.label.descriptionLimit"/></li></ul>', {
                    isHtml : true,
                    icon: 'error'
                });
                $("#id").focus();
            } else if (!idMatch) {
                UI.alert('<ui:msgEscJS key="console.directory.org.error.label.idInvalid"/>', {icon: 'error'});
                $("#id").focus();
            } else if (oversize) {
                UI.alert('<ui:msgEscJS key="console.directory.org.error.label.descriptionLimit"/>', {icon: 'error'});
            } else {
                $("#createOrg").submit();
            }
        }
