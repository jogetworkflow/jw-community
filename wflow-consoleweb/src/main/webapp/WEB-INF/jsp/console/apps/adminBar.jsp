...
<c:if test="${isQuickEditEnabled == 'true' || isQuickEditEnabled == 'false' || param.webConsole =='true'}">
    ...
    <c:set var="isSystemAdmin" value="<%= WorkflowUtil.isCurrentUserInRole(WorkflowUtil.ROLE_SYSTEMADMIN) %>"/>
    <c:set var="isAppAdmin" value="<%= WorkflowUtil.isCurrentUserInRole(WorkflowUtil.ROLE_APPADMIN) %>"/>
    <c:set var="isCustomAppAdmin" value="<%= EnhancedWorkflowUserManager.isAppDesignerRole() %>"/>
    <c:if test="${(isSystemAdmin || isAdmin || isAppAdmin) && (param.builderMode || !(isCustomAppAdmin && !empty param.webConsole))}">
        <script>
        ...
        <div id="adminBar" class="adminBarInactive" <c:if test="${deviceTheme}">device-theme="true"</c:if> <c:if test="${!empty theme}">builder-theme="<c:out value="${theme}"/>"</c:if>>
            ...
            <c:if test="${!empty param.appId && (isCustomAppAdmin || !isSystemAdmin)}">    
