...
PropertyEditor.Util = {
    ...
    retrieveOptionsFromCallback: function(properties) {
        try {
            if (properties.options_callback !== undefined && properties.options_callback !== null && properties.options_callback !== "" ) {
                var func = getFunction(properties.options_callback);
                if ($.isFunction(func)) {
        ...
        if (page !== null && page !== undefined) {
            ...
            $(pageContainer).find("[data-control_field][data-control_value]").each(function() {
                ...
                if (field !== null && field !== undefined) {
                    var control = $(page.editor).find("#"+field.id);
    ...
    callLoadOptionsAjax: function (field) {
        ...
        if(field.properties.options_ajax_on_change !== undefined && field.properties.options_ajax_on_change !== null){
            ...
            var targetValue = $(field.editor).find("#" + field.editorObject.fields[targetName].id).filter(":not(.hidden)").val();
            if(targetValue === null || targetValue === undefined){
                if(field.options.propertyValues !== undefined && field.options.propertyValues[targetName] !== undefined && field.options.propertyValues[targetName] !== ""){
    ...
    fieldOnChange: function (field) {
        var targetEl = $(field.editor).find("#" + field.editorObject.fields[field.properties.options_ajax_on_change].id);
...
PropertyEditor.Model.Editor.prototype = {
    ...
    adjustSize: function() {
        ...
        if ($(this.element).hasClass("boxy-content")) {
            ...
            tempHeight = tempHeight  * 0.95 - $(this.element).offset().top;
        }
...
PropertyEditor.Model.Type.prototype = {
    ...
    isHidden: function() {
        return $(".property_container_"+this.id).hasClass("hidden");
    },
    pageShown: function() {},
    remove: function() {}
};
...
PropertyEditor.Type.CheckBox.prototype = {
    ...
    getData: function(useDefault) {
        var data = new Object();
        var value = '';
        $('#'+ this.id + ':checkbox:checked').each(function(i){
            value += $(this).val() + ';';
        });
        if(value !== ''){
    ...
    renderField: function() {
        ...
        if(this.properties.options !== undefined && this.properties.options !== null){
            $.each(this.properties.options, function(i, option){
                ...
                html += '<span class="multiple_option"><label><input type="checkbox" id="'+ thisObj.id +'" name="'+ thisObj.id +'" value="'+PropertyEditor.Util.escapeHtmlTag(option.value)+'"'+checked+'/>'+option.label+'</label></span>';
            });
        }
        return html;
    },
    ...
    renderField: function() {
        ...
        if(this.properties.options !== undefined && this.properties.options !== null){
            $.each(this.properties.options, function(i, option){
                ...
                html += '<span class="multiple_option"><label><input type="radio" id="'+ thisObj.id +'" name="'+ thisObj.id +'" value="'+PropertyEditor.Util.escapeHtmlTag(option.value)+'"'+checked+'/>'+option.label+'</label></span>';
            });
        }
        return html;
    },
    ...
    renderField: function() {
        ...
        if(this.properties.options !== undefined && this.properties.options !== null){
            $.each(this.properties.options, function(i, option){
                ...
                html += '<option value="'+PropertyEditor.Util.escapeHtmlTag(option.value)+'"'+selected+'>'+option.label+'</option>';
            });
        }
        html += '</select>';
        return html;
        ...
        $.each(this.properties.columns, function(i, column) {
            ...
            if (column.options !== undefined) {
                ...
                $.each(column.options, function(i, option) {
                    html += '<option value="' + PropertyEditor.Util.escapeHtmlTag(option.value) + '">' + option.label + '</option>';
                });
                html += '</select>';
            } else {
