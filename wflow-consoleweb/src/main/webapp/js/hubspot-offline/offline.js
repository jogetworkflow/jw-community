...
(function () {
    ...
    extendNative = function (to, from) {
        ...
        if (null != handlers[event]) {
            for (ref = handlers[event].slice(0), results = [], j = 0, len = ref.length; j < len; j++)
                ref1 = ref[j],
        ...
        }, extendNative(window.XMLHttpRequest, _XMLHttpRequest), null != window.XDomainRequest)
            return _XDomainRequest = window.XDomainRequest,
    ...
    rc = Offline.reconnect = {}, retryIntv = null, reset = function () {
        ...
        if ("connecting" !== rc.state)
            return rc.remaining -= 1, Offline.trigger("reconnect:tick"),
                    0 === rc.remaining ? tryNow() : void 0;
    }, tryNow = function () {
        if ("waiting" === rc.state)
            return Offline.trigger("reconnect:connecting"), rc.state = "connecting",
                    Offline.check();
    }, down = function () {
        if (Offline.getOption("reconnect"))
            return reset(), rc.state = "waiting", Offline.trigger("reconnect:started"),
                    retryIntv = setInterval(tick, 1e3);
    }, up = function () {
        return null != retryIntv && clearInterval(retryIntv), reset();
    }, nope = function () {
        if (Offline.getOption("reconnect"))
            return "connecting" === rc.state ? (Offline.trigger("reconnect:failure"),
    ...
    held = [], waitingOnConfirm = !1, holdRequest = function (req) {
        ...
        if (xhr = arg.xhr, url = arg.url, type = arg.type, user = arg.user, password = arg.password,
            ...
            for (name in ref)
                val = ref[name], xhr.setRequestHeader(name, val);
        ...
        if (!1 !== Offline.getOption("requests")) {
            for (Offline.trigger("requests:flush"), requests = {}, i = 0, len = held.length; i < len; i++)
                request = held[i],
            ...
            for (key in requests)
                request = requests[key], makeRequest(request);
            return clear();
        }
    }, setTimeout(function () {
        if (!1 !== Offline.getOption("requests"))
            return Offline.on("confirmed-up", function () {
                if (waitingOnConfirm)
                    return waitingOnConfirm = !1, clear();
                ...
                if (xhr = request.xhr, async = request.async, !1 !== xhr.offline && (hold = function () {
                    ...
                    return null === xhr.onprogress ? (xhr.addEventListener("error", hold, !1),
