	...
	private static class SpringLocalizationContext extends LocalizationContext {
		...
		public SpringLocalizationContext(MessageSource messageSource, HttpServletRequest request) {
			this.messageSource = messageSource;
                        // CUSTOM: custom wrapper to catch possible NullPointerException for async requests in openliberty
			this.request = new HttpServletRequestWrapper(request) {
                            @Override
                            public HttpSession getSession(boolean create) {
                                // handle possible NullPointerException for async requests in openliberty
                                try {
		...
		public ResourceBundle getResourceBundle() {
			...
			if (session != null) {
				Object lcObject = Config.get(session, Config.FMT_LOCALIZATION_CONTEXT);
                                // CUSTOM: Don't use instanceof expression to avoid aspectj-maven-plugin compilation error
