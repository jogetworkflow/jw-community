...
public class LocalLocaleResolver extends SessionLocaleResolver implements LocaleResolver, I18nResourceProvider{
    ...
    protected TimeZone determineDefaultTimeZone(HttpServletRequest request) {
        ...
        if (request != null) {
            ...
            if (timezone == null) {
                // lookup in session
                Long defaultExpiry = (Long)request.getSession().getAttribute(SYSTEM_TIMEZONE_EXPIRY_KEY);
                if (defaultExpiry == null || defaultExpiry.compareTo(System.currentTimeMillis()) < 0) {
                    request.getSession().removeAttribute(SYSTEM_TIMEZONE);
                } else {
                    timezone = (TimeZone) request.getSession().getAttribute(SYSTEM_TIMEZONE);
                    request.setAttribute(SYSTEM_TIMEZONE, timezone);
                }
            }
            
            if (timezone != null && !getWorkflowUserManager().getCurrentUsername().equals(request.getSession().getAttribute(TIMEZONE_OF_USER))) {
                timezone = null;
            }
        }
            ...
            try {
                ...
                if (gmt != null && !gmt.isEmpty()) {
                    ...
                    if (timezone != null && request != null) {
                        ...
                        request.getSession().setAttribute(SYSTEM_TIMEZONE_EXPIRY_KEY, expiry);
                        request.getSession().setAttribute(SYSTEM_TIMEZONE, timezone);
                    }
                }
            } catch (Exception e) {
        ...
        if (request != null) {
            request.getSession().setAttribute(TIMEZONE_OF_USER, getWorkflowUserManager().getCurrentUsername());
        }
    ...
    protected Locale determineDefaultLocale(HttpServletRequest request){
        ...
        if (request != null) {
            ...
            if (locale == null) {
                // lookup in session
                Long defaultExpiry = (Long)request.getSession().getAttribute(DEFAULT_LOCALE_EXPIRY_KEY);
                if (defaultExpiry == null || defaultExpiry.compareTo(System.currentTimeMillis()) < 0) {
            ...
            if (locale != null && !getWorkflowUserManager().getCurrentUsername().equals(request.getSession().getAttribute(LOCALE_OF_USER))) {
                locale = null;
            }
        }
        ...
        if (request != null) {
            request.getSession().setAttribute(LOCALE_OF_USER, getWorkflowUserManager().getCurrentUsername());
        }
    ...
    public Locale resolveLocale(HttpServletRequest request) {
        ...
        if (request != null) {
            ...
            if (request.getParameter(PARAM_NAME) != null 
                    && (locale == null || (locale != null && !request.getParameter(PARAM_NAME).equals(locale.toString())))) {
                locale = null;
