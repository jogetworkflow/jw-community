...
public class ProcessBuilderWebController {
    ...
    public String save(Writer writer, HttpServletRequest request, @RequestParam("appId") String appId, @RequestParam(value = "version", required = false) String version, @RequestParam("json") String json) throws Exception {
        ...
        try {
            if (!oriJson.equals(json)) {
                ...
                //compare mapping
                if (success) {
                    updateMapping(oriObject, newObject, appDef);
                }
            }
        } catch (Exception e) {
            success = false;
    ...
    @Transactional
    protected void updateMapping(JSONObject oriObject, JSONObject newObject, AppDefinition appDef) throws Exception {
        PackageDefinition packageDef = packageDefinitionDao.loadAppPackageDefinition(appDef.getAppId(), appDef.getVersion());
        if (packageDef == null) {
